...
class Main:
    ...
    def __init__(self):
        ...
        self.bCastle = "kq"
        self.lastMove = None
        self.checkMateFound = False
        self.chessActive = False
        self.bestMoves = {
            'w': [],
            'b': [],
        }
        ...
        if not len(self.pieces):
            edp = "6k1/pp3r2/2p4q/3p2p1/3Pp1b1/4P1P1/PP4RP/2Q1RrNK b"
            self.deployEdp(edp)
        ...
        self.currentStanding = self.whitePieceW + self.blackPieceW
    ...
    def drawTable(self):
        for i in range(8):
            ...
            for j in range(8):
                p.draw.rect(self.screen, tableColor, self.getTableRect(
                    self.addon + j*self.cellWidth, self.addon / self.heightOptimizer + i*self.cellWidth))
    ...
    def drawBorder(self):
        ...
        for k in range(2):
            p.draw.rect(self.screen, borderColor, p.Rect(
                self.addon, self.addon / self.heightOptimizer+k*rectSize, rectSize, 5))
            p.draw.rect(self.screen, borderColor, p.Rect(
                self.addon+k*rectSize, self.addon / self.heightOptimizer, 5, rectSize+5))
 ...
 ####################
    ...
    def createPawn(self, pawnNum, color):
        ...
        pawn.x, pawn.y = self.addon + self.cellWidth * pawnNum + \
            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
            self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
        pawn.rect.x = pawn.x
        pawn.rect.y = pawn.y
 ...
 ####################
    ...
    def createRook(self, rookNum, color):
        ...
        rook.x, rook.y = self.addon + self.cellWidth * rookNum + \
            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
            self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
        rook.rect.x = rook.x
        rook.rect.y = rook.y
 ...
 ####################
    ...
    def createKnight(self, knightNum, color):
        ...
        knight.x, knight.y = self.addon + self.cellWidth * knightNum + \
            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
            self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
...
####################
    ...
    def createBishop(self, bishopNum, color):
        ...
        bishop.x, bishop.y = self.addon + self.cellWidth * bishopNum + \
            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
            self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
...
####################
    ...
    def createKings(self, kingNum, color):
        ...
        king.x, king.y = self.addon + self.cellWidth * kingNum + \
            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
            self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
        king.rect.x = king.x
        king.rect.y = king.y
...
####################
    ...
    def createQueens(self, queenNum, color):
        ...
        queen.x, queen.y = self.addon + self.cellWidth * queenNum + \
            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
            self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
...
####################
    ...
    def movePieceToNearestSquare(self, x, y):
        ...
        self.clickedPiece.rect.x, self.clickedPiece.rect.y = self.addon + self.cellWidth * x + \
            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
            self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
    ...
    def typeOfMove(self, oldX, oldY, newX, newY, piece):
        ...
        if typeOfPiece == "king":
            ...
            if oldX == newX:
                if not self.checkPawnDirection(oldY, newY, piece):
                    return False
                return "vertical"
        ...
        if typeOfMove == "horizontal":
            if oldX < newX:
                for piece in self.pieces:
                    pieceX, pieceY = piece.getCurrentPos(
    ...
    def getAllPossibleMoves(self, color):
        ...
        for piece in self.pieces:
            if piece.color == color:
                ...
                # THE PIECE, # CURR POS
                tempList = [piece, [oldpos[0], oldpos[1]]]
    ...
    def promotePawn(self, pawn, choosenPiece, x, y):
        if pawn.piece == "pawn":
            ...
            choosenPiece.x, choosenPiece.y = self.addon + self.cellWidth * x + \
                int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
                self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
    ...
    def officialMoves(self, piece=None, color=None):
        ...
        if piece:
                ...
                if pieceInfo[0] == piece:
                    ...
                    if length > 2:
                        for i in range(2, length):
                            ...
                            elif color not in self.chess:
                                officialMoves.insert(0, [pieceInfo[i][0], pieceInfo[i][1]])
            ...
            if piece.piece == "king":
                ...
                if not self.chessActive or color not in self.chessActive:
                    if not piece.moved:
                                        ...
                                        for rook in self.pieces:
                                            if rook.piece == "rook" and rook.color == color:
                                                ...
                                                if rookX < oldX:
                                                    if not rook.moved:
                                                        flagL = True
                if not flagL and not piece.moved:
                    if [2, oldY] in officialMoves:
                        officialMoves.remove([2, oldY])
                if not flagR and not piece.moved:
            ...
            for i in range(len(moves)):
                ...
                if length > 2:
                    for j in range(2, length):
                        piece.rect.x, piece.rect.y = self.addon + self.cellWidth * moves[i][j][0] + int(
                            self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + self.cellWidth * moves[i][j][1] + int(self.settings.cellWidth * 0.1) - 5
                        self.pieces.move_to_front(piece)

                        if piece.piece == "king":
                            newX, newY = piece.getCurrentPos(
                                piece.rect.x, piece.rect.y)
                            typeOfMove = self.typeOfMove(
                                oldX, oldY, newX, newY, piece)
                            if typeOfMove == "castle":
                                self.handleCastling(
                                    self.turn, oldX, newX, newY)
                        ...
                        if piece.piece == "king" and typeOfMove == "castle":
                            self.moveBackCastle(piece)
                        piece.rect.x, piece.rect.y = self.addon + self.cellWidth * oldX + \
                            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
                            self.cellWidth * oldY + \
                            int(self.settings.cellWidth * 0.1) - 5
    ...
    def moveBackCastle(self, king):
        x, y = king.getCurrentPos(king.rect.x, king.rect.y)
        for rook in self.pieces:
            if rook.piece == "rook" and rook.color == king.color:
                rookX, rookY = rook.getCurrentPos(rook.rect.x, rook.rect.y)
                if rookY == y:
                    if rookX - x == 1:
                        rook.rect.x, rook.rect.y = self.addon + self.cellWidth * 0 + \
                            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
                            self.cellWidth * y + \
                            int(self.settings.cellWidth * 0.1) - 5
                    elif x - rookX == 1:
                        rook.rect.x, rook.rect.y = self.addon + self.cellWidth * 7 + \
                            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
                            self.cellWidth * y + \
                            int(self.settings.cellWidth * 0.1) - 5
    ...
    def colorPossibleSquares(self, piece, chess=False, kX=None, kY=None):
        ...
        if piece:
            if self.turn == piece.color:
                ...
                p.draw.rect(self.screen, self.settings.pieceSquare,
                            self.squares[posX + posY * 8])
                if self.possibleMoves:
                    for val in self.possibleMoves:
                        p.draw.rect(
                            self.screen, self.settings.moveableSquares, self.squares[val[0] + val[1] * 8])

    def checkPawnDirection(self, oldY, newY, piece=None):
        if not piece:
            piece = self.clickedPiece
        if piece.color == "w":
            if oldY < newY:
    ...
    def getPossibleMoves(self, piece, flag=False):
        ...
        if not flag:
            ...
            currPieceX, currPieceY = piece.getCurrentPos(
                piece.rect.x, piece.rect.y)
        if typeOfPiece == "king":
            if not currPiece.moved:
                ...
                if len(rooks):
                    for rook in rooks:
                        between = False
                        castleRookX, castleRookY = rook.getCurrentPos(
                            rook.rect.x, rook.rect.y)
                        for piece in self.pieces:
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    if 0 <= currPieceX + i < 8 and 0 <= currPieceY + j < 8:
                        if i != 0 or j != 0:
                            possibleMoves.append(
                                [currPieceX + i, currPieceY + j])
            for otherPiece in self.pieces:
                currentPos = list(otherPiece.getCurrentPos(
                    otherPiece.rect.x, otherPiece.rect.y))
            ...
            for otherPiece in self.pieces:
                ...
                if currentPos in possibleMoves and currentPos[0] == currPieceX:
                    possibleMoves.remove(currentPos)
                    if not currPiece.moved and ([currPieceX, currPieceY+2] in possibleMoves or [currPieceX, currPieceY-2] in possibleMoves):
                        try:
                            possibleMoves.remove([currPieceX, currPieceY-2])
                        except:
                            possibleMoves.remove([currPieceX, currPieceY+2])
            ...
            if self.freshlyMovedPawn and currPieceY == 3 and color == "w":
                x, y = self.freshlyMovedPawn.getCurrentPos(
                    self.freshlyMovedPawn.rect.x, self.freshlyMovedPawn.rect.y)
            ...
            if typeOfPiece in ["rook", "queen"]:
                ...
                for otherPiece in self.pieces:
                    if otherPiece != currPiece:
                        otherPieceX, otherPieceY = otherPiece.getCurrentPos(
                            otherPiece.rect.x, otherPiece.rect.y)
            ...
            if typeOfPiece in ["bishop", "queen"]:
                for y in range(8):
                    for x in range(8):
                        move = self.typeOfMove(
                            currPieceX, currPieceY, x, y, currPiece)
                        if move == "diagonal":
        ...
        for otherPiece in self.pieces:
            if otherPiece.color == color:
                if otherPiece != self.clickedPiece:
                    posX, posY = otherPiece.getCurrentPos(
                        otherPiece.rect.x, otherPiece.rect.y)
    ...
    def handleCastling(self, color, oldX, newX, kingY):
        for piece in self.pieces:
            if piece.piece == "rook" and piece.color == color:
                x, y = piece.getCurrentPos()
                if oldX < newX:
                    if [x, y] == [7, kingY]:
                        piece.rect.x, piece.rect.y = self.addon + self.cellWidth * 5 + \
                            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
                            self.cellWidth * kingY + \
                            int(self.settings.cellWidth * 0.1) - 5
                        return True
                else:
                    if [x, y] == [0, kingY]:
                        piece.rect.x, piece.rect.y = self.addon + self.cellWidth * 3 + \
                            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
                            self.cellWidth * kingY + \
                            int(self.settings.cellWidth * 0.1) - 5
                        return True
    ...
    def handleAmpass(self, killedPawn, pawn):
        ...
        killedPawnX, killedPawnY = killedPawn.getCurrentPos(
            killedPawn.rect.x, killedPawn.rect.y)
    ...
    def visualizeMove(self, piece, x, y):
        piece.rect.x, piece.rect.y = self.addon + self.cellWidth * x + \
            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
            self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
        self.pieces.move_to_front(piece)
        collision = self.checkCollision(piece)
    ...
    def minimaxRoot(self, depth, isMaximisingPlayer):
        ...
        for piece in self.pieces:
            if piece.piece == "queen":
                self.pieces.move_to_back(piece)
            ...
            if currPiece.color == self.ai.color:
                ...
                if moves:
                    for move in moves:
                        ...
                        newMoveVal = self.minimax(depth - 1, -10000, 10000, not isMaximisingPlayer)
                        ...
                        if collision:
                            self.pieces.add(collision)
                        print(newMoveVal, self.translatePosition(move[0], move[1]), currPiece.piece)
                        if newMoveVal == -9999:
                            return currPiece, move
                        if newMoveVal >= bestMoveVal:
                            bestMoveVal = newMoveVal
                            bestMove = currPiece, move
        self.currentStanding = -bestMoveVal
        return bestMove

    def minimax(self, depth, alpha, beta, isMaximisingPlayer):
        self.positionsChecked += 1
        if depth == 0 or self.checkMateFound:
            worth = self.ai.getPieceWorth(self.pieces)
            self.currentStanding = worth[0] + worth[1]
                ...
                if currPiece.color != self.ai.color:
                    ...
                    if moves:
                        for move in moves:
                            ...
                            collision = self.visualizeMove(
                                currPiece, move[0], move[1])
                            bestMove = min(bestMove, self.minimax(depth - 1, alpha, beta, not isMaximisingPlayer))
                            currPiece.rect.x, currPiece.rect.y = self.addon + self.cellWidth * oldX + int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + self.cellWidth * oldY + int(self.settings.cellWidth * 0.1) - 5
                            if collision:
                                self.pieces.add(collision)
                            if bestMove == 9999:
    ...
    def createEPD(self):
        ...
        for i in range(8):
            ...
            for k in range(8):
                ...
                if k == 7:
                    if emptycounter > 0:
                        if i == 0 or type(edp[-1]) == str:
                            edp += f"{emptycounter}"
                        else:
                            edp += str(emptycounter)
                    if i == 0 or (edp[-1] != "/" and i != 7):
                        edp += "/"
        if self.wCastle == " " and self.bCastle == " ":
            edp += f" b - - 10 1"
        else:
            edp += f" b {self.wCastle}{self.bCastle} - 10 1"
        #print(edp)
        return edp


    def deploySinglePiece(self, pieceName, pieceColor, x, y):
        pieceName = Piece(pieceName, pieceColor)
        pieceName.x, pieceName.y = self.addon + self.cellWidth * x + \
            int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
            self.cellWidth * y + int(self.settings.cellWidth * 0.1) - 5
        pieceName.rect.x = pieceName.x
        pieceName.rect.y = pieceName.y
        self.pieces.add(pieceName)


    def deployEdp(self, edp):
        x = 0
        y = 0
        isFEN = True
        flagW = False
        ...
        for c in edp:
            ...
            elif type(c) == int and isFEN and c != 8:
                x += c
                continue
            
            elif isinstance(c, str) and isFEN and c != "/":
                if c.isupper():
                    self.deploySinglePiece(pieceNames[c.lower()],"w",x, y)
                else:
                    self.deploySinglePiece(pieceNames[c],"b",x, y)
                x += 1
            ...
            if not isFEN:
                if c == "b":
                    ...
                    for piece in self.pieces:
                        if piece.color == "w" and piece.piece == "king":
                            piece.moved = False
                            flagW = True
                elif c == "k" or c == "q":
                    for piece in self.pieces:
                        if piece.color == "b" and piece.piece == "king":
                            piece.moved = False
                            flagB = True
        ...
        if not flagB:
            for piece in self.pieces:
                if piece.color == "b" and piece.piece == "king":
                    piece.moved = True
    def handleComputerOpeningsMove(self, move):
        move = str(move)
        if len(move) == 4:
            oldPosX, oldPosY = int(self.settings.boardLetters.index(move[0])), int(
                self.settings.boardNums[::-1].index(move[1]))
            newPosX, newPosY = int(self.settings.boardLetters.index(move[2])), int(
                self.settings.boardNums[::-1].index(move[3]))

        for piece in self.pieces:
            if piece.color == self.ai.color:
                x, y = piece.getCurrentPos(piece.rect.x, piece.rect.y)
                if (oldPosX + oldPosY * 8) == (x + y * 8):
    ...
    def aiMove(self):
        ...
        if self.numOfTurn > 15:
            ...
            moves = []
            with openings.open_reader("C:\\programming\\python\\projects\\chess_engine_project\\chess\\bin\\openings.bin") as reader:
                for entry in reader.find_all(board):
                    moves.append(entry.move)
            if moves:
                piece, newX, newY, oldX, oldY = self.handleComputerOpeningsMove(
                    str(choice(moves)))
                piece.rect.x, piece.rect.y = self.addon + self.cellWidth * newX + \
                    int(self.settings.cellWidth * 0.1), self.addon / self.heightOptimizer + \
                    self.cellWidth * newY + \
                    int(self.settings.cellWidth * 0.1) - 5
                self.pieces.move_to_front(piece)
            ...
            if event.type == p.QUIT:
                ...
                if event.key == p.K_r:
                    if self.promotion and self.newPos and self.clickedPiece:
                        self.promotePawn(
                            self.clickedPiece, "rook", self.newPos[0], self.newPos[1])
                if event.key == p.K_q:
                    if self.promotion and self.newPos and self.clickedPiece:
                        self.promotePawn(
                            self.clickedPiece, "queen", self.newPos[0], self.newPos[1])
                if event.key == p.K_b:
                    if self.promotion and self.newPos and self.clickedPiece:
                        self.promotePawn(
                            self.clickedPiece, "bishop", self.newPos[0], self.newPos[1])
                if event.key == p.K_k:
                    if self.promotion and self.newPos and self.clickedPiece:
                        self.promotePawn(
                ...
                for piece in self.pieces:
                    if piece.check_click(event.pos):
                        ...
                        if self.flag:
                            self.wKingRow, self.wKingColumn, self.bKingRow, self.bKingColumn = self.getKingPos(
                                "w")[0], self.getKingPos("w")[1], self.getKingPos("b")[0], self.getKingPos("b")[1]
                ...
                if self.clickedPiece and self.dragging:
                    ...
                    if not self.clickedPiece.isOutofTable(posX, posY) and self.clickedPiece.color == self.turn:
                        oldPos = self.clickedPiece.getCurrentPos(
                            self.oldX, self.oldY)
                        ...
                        if newPos != self.clickedPiece.getCurrentPos(self.oldX, self.oldY):
                            ...
                            if self.possibleMoves:
                                if [newPos[0], newPos[1]] in self.possibleMoves:
                                    typeOfMove = self.typeOfMove(
                                        oldPos[0], oldPos[1], newPos[0], newPos[1], self.clickedPiece)
                                    if typeOfMove:
                                        if self.validateMove(self.clickedPiece, typeOfMove, self.clickedPiece.checkHit(self.clickedPiece, self.pieces, True)):
                                            if not self.isSteppedOverAnotherPiece(oldPos[0], oldPos[1], newPos[0], newPos[1], typeOfMove):
                                                if typeOfMove == "castle":
                                                    self.wCastle = " "
                                                    self.handleCastling(
                                                        self.turn, oldPos[0], newPos[0], newPos[1])
    ...
    def run(self):
        ...
        if self.colorWKing and not self.colorBKing and self.turn == "w":
            self.colorPossibleSquares(
                self.clickedPiece, True, self.wKingRow, self.wKingColumn)
        elif self.colorBKing and not self.colorWKing and self.turn == "b":
            self.colorPossibleSquares(
                self.clickedPiece, True, self.bKingRow, self.bKingColumn)

        if self.clickedPiece:
            self.colorPossibleSquares(self.clickedPiece)
        if self.lastMove and not self.clickedPiece:
        ...
        # CHECKMATE
        if self.checkmate:
            self.screen.blit(self.checkmateFont.render(
                f"Game End! {self.checkmate} Won!", False, (255, 0, 0), (0, 0, 0)), (200, self.screen.get_height() / 6))
        elif self.stalemate:
            self.screen.blit(self.checkmateFont.render(f"Game End! It's a draw!",
                             False, (255, 0, 0), (0, 0, 0)), (200, self.screen.get_height() / 6))
            # RESTART GAME, QUIT....

        # PIECE WORTH
        if self.checkmate:
            currentStanding = "Game Over"
        elif self.stalemate:
            currentStanding = "0"
        else:
            currentStanding = str((self.currentStanding) / 10)
        self.screen.blit(self.infoFont.render(currentStanding, False,
                         (0, 0, 0)), (self.addon / 2, self.screen.get_height() / 2))

        # PROMOTION
        if self.promoted:
            self.screen.blit(self.promotion.render(f"PRESS 'Q' FOR QUEEN", False,
                             (255, 150, 20), (0, 110, 50)), (0, self.screen.get_height() / 4))
            self.screen.blit(self.promotion.render(f"PRESS 'R' FOR ROOK", False,
                             (255, 150, 20), (0, 110, 50)), (300, self.screen.get_height() / 4))
            self.screen.blit(self.promotion.render(f"PRESS 'B' FOR BISHOP", False,
                             (255, 150, 20), (0, 110, 50)), (600, self.screen.get_height() / 4))
            self.screen.blit(self.promotion.render(f"PRESS 'K' FOR KNIGHT", False,
                             (255, 150, 20), (0, 110, 50)), (900, self.screen.get_height() / 4))
